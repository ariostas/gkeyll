...
t_end = 0.01 / nu -- Final simulation time.
...
gyrokineticApp = Gyrokinetic.App.new {
  ...
  square = Gyrokinetic.Species.new {
    ...
    mapc2p = {
      -- Rescaled top hat velocity space coordinates (vpar, mu) from old velocity space coordinates (cpvar, cmu).
  ...
  bump = Gyrokinetic.Species.new {
    ...
    mapc2p = {
      -- Rescaled bump velocity space coordinates (vpar, mu) from old velocity space coordinates (cpvar, cmu).
      mapping = function (t, vc)
        local cvpar, cmu = vc[1], vc[2]
